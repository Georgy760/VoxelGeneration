...
public class World : MonoBehaviour
{
    ...
    public void GenerateWorld()
    {
        ...
        foreach (ChunkData data in _chunkDataDictionary.Values)
        {
            ...
            GameObject chunkObject = Instantiate(chunkPrefab, data.worldPosition, Quaternion.identity, this.transform);
            chunkObject.name += data.worldPosition;
        ...
        private void GenerateVoxels(ChunkData data)
    {
        for (int x = 0; x < data.chunkSize; x++)
        {
            ...
            {
                Color pixel = heightMap.GetPixel(data.worldPosition.x + x, data.worldPosition.z + z);
                float heightValue = pixel.r;
                int groundPosition = Mathf.RoundToInt(heightValue * chunkHeight);
